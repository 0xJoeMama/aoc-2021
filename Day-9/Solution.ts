...
const isLow = (pos: Position, nums: number[][]) =>
  getNeighbours(nums, pos.x, pos.y)
...
function part1(): number {
  const numbers: number[][] = input.map((it) =>
    it.split("").map((itt) => Number.parseInt(itt))
  );
  return numbers.map(
...
function part2(): number {
  ...
  const getBasin = (
    ...
    neighbours.forEach((neighbour) =>
      knownPoses[posToString(neighbour)] = true
    );
