...
const parseInsertions = (insertions: string[]): InsertionMap => {
  ...
  while (insertions.length > 0) {
    const [pair, insert] = (insertions.pop() ?? "").split(/->/).map((it) =>
      it.trim()
    );
    ret[pair] = insert;
  }
...
const countAtoms = (results: ResultMap): number => {
  ...
  return Math.floor(
    sortedValues[sortedValues.length - 1] / 2 - sortedValues[0] / 2,
  );
};

function part1(): number {
  const polymer = input[0];
  const insertions = parseInsertions(
    input.slice(1).map((it) => it.trim()).filter((it) => it !== ""),
  );
  const res = step(polymer, insertions, 10);

  console.log(
    "If this result doesn't work, try either one above or one below it",
  );
