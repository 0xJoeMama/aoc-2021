...
class OperatorPacket implements Packet {
  ...
  constructor(
    ...
    this.bitSize = this.children.map((it) => it.bitSize).reduce(sum) + 7 +
      (this.lengthType ? 11 : 15);

    switch (this.lengthType) {
      case LengthType.LITERAL:
        if (
          this.children.map((it) => it.bitSize).reduce(sum) !==
            this.specialNumber
        ) {
  ...
  private parseChildren(content: string): Packet[] {
    ...
    while (
      ...
      (this.lengthType || i < this.specialNumber)
    ) {
      if (
        currentRemainingString.split("").every((it) => it === "0") ||
        (this.lengthType === LengthType.COUNT &&
          ret.length === this.specialNumber)
      ) {
        return ret;
      }
  ...
  public static parseOperator(str: string): OperatorPacket | undefined {
    ...
    caret += 3;
    const lengthType: LengthType = parseInt(
      str.substr(caret, 1),
      2,
    ) as LengthType;
    caret += 1;
...
const getBin = (input: string): string =>
  input
    .split("")
...
function part1(): number {
  const getVersionSum = (packet: Packet): number => {
    ...
    return packet.version +
      (packet as OperatorPacket).children.map((it) => getVersionSum(it)).reduce(
        sum,
      );
  };
