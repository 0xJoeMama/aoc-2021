export class Setup {
    ...
    async createRunConfig(decoder: TextDecoder, encoder: TextEncoder, cwd = Deno.cwd()) {
        const bytes = await Deno.readFile(`${cwd}/setup/Day-n/Solution.ts.run.xml.template`);
