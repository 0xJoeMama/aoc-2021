...
function part1(): number {
  ...
  return input.map((it) => it.split(/\|/))
    .map((it) => it[1])
    .map((it) =>
      it.trim().split(/ /).filter((num) => uniqueLengths.includes(num.length))
    )
...
function part2(): number {
  ...
  const countMatchingPoints = (measurement: string, key: string) =>
    key.split("").reduce(
      (acc, it) => acc + (measurement.includes(it) ? 1 : 0),
      0,
    );
  const containsAll = (test: string, seq: string) =>
    test.split("").every((char) => seq.includes(char)) &&
    test.length === seq.length;
  ...
  const parseSixDigits = (keys: Key, measurements: string[]) => {
    ...
    keys[0] = sixDigits.find((it) =>
      it !== keys[9] && contains(it, keys[1])
    ) as string;
    keys[6] = sixDigits.find((it) =>
      it !== keys[9] && it !== keys[0]
    ) as string;
  };
  ...
  const parseFiveDigits = (keys: Key, measurements: string[]) => {
    ...
    keys[5] = fiveDigits.find((it) =>
      it !== keys[3] && countMatchingPoints(it, keys[9]) === 5
    ) as string;
    keys[2] = fiveDigits.find((it) =>
      it !== keys[3] && it !== keys[5]
    ) as string;
  };
  ...
  const findValue = (key: Key, sequence: string) => {
    const foundEntry = Object.entries(key).find((it) =>
      containsAll(sequence, it[1])
    );
  ...
  const measurements: string[][] = input.map((it) =>
    it.split(/\|/)[0].trim().split(/ /)
  )
    .map((it) => it.sort((s1, s2) => s1.length - s2.length));
  const keys: Key[] = measurements.map((it) => parseKeys(it));
  const outputs: string[][] = input.map((it) =>
