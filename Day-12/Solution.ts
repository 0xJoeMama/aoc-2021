...
import { Stringable, StringMap, StringSet } from "../util/DataLib.ts";
...
function part1(): number {
  const findPaths = (
    start: Cave,
    visited: StringSet<Cave>,
    callback: () => void,
  ) => {
    const validChildren = start.children.filter((it) =>
      it.value !== "start" && (it.isBig || !visited.contains(it))
    );
    if (start.value === "end") {
      callback();
      return;
...
function part2(): number {
  const findPaths = (
    ...
    const canDoDouble = visited.entries().filter((it) =>
      it[0] !== it[0].toUpperCase()
    )
      .filter((it) => it[0] !== start.value).map((it) => it[1]).every(
        (timesVisited) => timesVisited < 2,
      );
    if (!start.isBig && (visited.get(start) ?? 0) === 1 && !canDoDouble) return;
    const validChildren = start.children
      .filter((it) =>
        it.value !== "start" && (it.isBig || (visited.get(it) ?? 0) < 2)
      );
