...
class Board {
  ...
  crossOut(number: number): boolean {
    if (this._cells.get(number) !== nullPos) {
      ...
      throw new Error(
        `Number ${number} has already been crossed out on board ${this._index}`,
      );
    }
  }
  ...
  countScore(lastRng: number): number {
    return lastRng *
      [...this.cells.entries()].filter((cell) => cell[1] !== nullPos)
        .map((it) => it[0])
        .reduce(sum);
  }
  ...
  private initializeCells() {
    ...
    for (let i = 0, lines = boardString.split(/,/); i < lines.length; i++) {
      for (
        let j = 0, cells = lines[i].split(/ /).filter((it) => it !== "");
        j < cells.length;
        j++
      ) {
  ...
  private wonHorizontal(): boolean {
    for (let i = 0; i < Board.size.ySize; i++) {
      if (
        [...this._cells].filter((entry) => entry[1].y === i).map((it) => it[1])
          .every((it) => it === nullPos)
      ) {
        return true;
      }
  ...
  private wonVertical(): boolean {
    for (let i = 0; i < Board.size.xSize; i++) {
      if (
        [...this._cells].filter((entry) => entry[1].x === i).map((it) => it[1])
          .every((it) => it === nullPos)
      ) {
        return true;
      }
...
function part1(): number {
  ...
  for (const i of randomNumbers) {
    const boardThatWon: Board | undefined = boards.find((board) =>
      board.crossOut(i)
    );
