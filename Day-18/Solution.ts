...
const parseAsSnailfishNumber = (line: string) => {
  const number: SnailfishNumber = { values: [] };
  let currDepth = 0;
  for (
    const char of line.split("").map((it) => it.trim()).filter((it) =>
      it !== ","
    )
  ) {
    if (char === "[") {
...
const reduce = (invalid: SnailfishNumber): SnailfishNumber => {
  ...
  do {
    ...
    if (explode >= 0) {
      ...
      copy.values.splice(explode, 2, {
        value: 0,
        depth: explodingPair[0].depth - 1,
      });
    } else if (split >= 0) {
...
const ins = (value: number, depth: number, tree: BinNode): boolean => {
  if (depth === 0) {
    if (
      tree.value === undefined && tree.left === undefined &&
...
function part2(): number {
  ...
  for (let i = 0; i < snailfishNumbers.length; i++) {
    for (let j = 0; j < snailfishNumbers.length; j++) {
      if (i !== j) {
        results.push(
          calcMagnitude(add(snailfishNumbers[i], snailfishNumbers[j])),
        );
      }
    }
  }
