...
const parseFold = (foldLine: string): Fold => {
    ...
    return <Fold>{axis: importantPart[0], foldPoint: Number.parseInt(importantPart.slice(2))};
};
...
function part1(): number {
    const coords = input.slice(0, input.findIndex(it => it === ''))
        ...
        .reduce(collectToSet, {} as CustomSet);
    return Object.values(fold(coords, parseFold(input.find(it => it.startsWith('fold')) ?? ''))).length;
}
